...
export default function Home() {
  return (
    <div className="flex flex-col items-center w-full">
      ...
      {/* Product Overview */}
      <section className="w-full py-32 relative z-10 -mt-32 border-t border-black/5 dark:border-white/[0.05] bg-transparent overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
          <div className="flex flex-col md:flex-row gap-16 mb-24 items-end">
            <div className="flex-1">
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-blue-500/[0.03] border border-blue-500/20 text-blue-500 dark:text-blue-400 text-[10px] font-mono font-bold tracking-[0.2em] uppercase mb-8">
                <Target className="w-3.5 h-3.5" />
                SYSTEM.INTEL.ARCHITECTURE // v2.0
              </div>
              <h2 className="text-4xl md:text-6xl font-mono font-black uppercase text-black dark:text-white tracking-tighter leading-[0.9]">
                Core Platform Capability
              </h2>
            </div>
            <div className="flex-1 pb-2 flex flex-col md:items-end gap-6 text-left md:text-right">
              <p className="text-sm text-gray-500 dark:text-gray-400 font-mono tracking-wide leading-relaxed uppercase border-l md:border-l-0 md:border-r border-blue-500/30 pl-6 md:pl-0 md:pr-6">
              ...
              <Link 
                href="/geofence" 
                className="inline-flex items-center gap-2 justify-center px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-mono font-bold uppercase tracking-widest text-xs transition-colors"
                >
                Launch Platform
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-px bg-black/5 dark:bg-white/[0.05] border border-black/5 dark:border-white/[0.05] p-px">
            <div className="md:col-span-8 bg-gray-50 dark:bg-[#0a0a0a]">
              <FeatureCard
            ...
            <div className="md:col-span-4 bg-gray-50 dark:bg-[#0a0a0a]">
              <FeatureCard
      ...
      <section className="w-full py-32 bg-transparent border-t border-black/5 dark:border-white/[0.05] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-24 relative z-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-px bg-black/5 dark:bg-white/[0.05] border border-black/5 dark:border-white/[0.05]">
...
function FeatureCard({ icon, title, description, metric, accent }: { icon: React.ReactNode, title: string, description: string, metric: string, accent: string }) {
  return (
    <div className="group relative h-full bg-gray-50 dark:bg-[#0a0a0a] p-10 transition-colors hover:bg-gray-100 dark:hover:bg-white/[0.02] flex flex-col justify-between">
      <div>
        <div className="mb-8 p-3 border border-black/10 dark:border-white/10 inline-flex transition-colors group-hover:border-black/30 dark:group-hover:border-white/30 text-black dark:text-white">
          <div className={`transition-colors ${accent}`}>
            {icon}
          </div>
        </div>
        <h3 className="text-xl font-mono font-bold uppercase text-black dark:text-white mb-4 tracking-tight">{title}</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 font-sans leading-relaxed">{description}</p>
      </div>
      <div className="mt-12 pt-4 border-t border-black/5 dark:border-white/[0.05] flex justify-between items-center">
...
function ProgressItem({ title, status }: { title: string, status: 'completed' | 'in-progress' | 'pending' }) {
  ...
  return (
    <div className="group relative flex items-center justify-between p-4 border border-black/5 dark:border-white/5 bg-gray-50 dark:bg-black/40 hover:bg-gray-100 dark:hover:bg-white/[0.02] hover:border-black/20 dark:hover:border-white/20 transition-all">
      <div className="flex items-center gap-4">
        <span className="text-xs font-mono text-gray-600 tracking-widest uppercase">
          {isComplete ? '[OK]' : '[..]'}
        </span>
        <h4 className={`text-sm font-mono tracking-wide uppercase ${isComplete ? 'text-gray-700 dark:text-gray-300' : 'text-blue-500 dark:text-blue-400 animate-pulse'}`}>
          {title}
        </h4>
      </div>
      <div className={`w-2 h-2 border ${isComplete ? 'bg-black/20 dark:bg-white/20 border-black/40 dark:border-white/40' : 'bg-blue-500/50 border-blue-500 dark:border-blue-400'} transition-colors`} />
    </div>
  );
}
...
function TechBadge({ name, type }: { name: string, type: string }) {
  return (
    <div className="bg-gray-50 dark:bg-[#0a0a0a] hover:bg-gray-100 dark:hover:bg-white/[0.02] p-6 transition-colors flex flex-col justify-between h-32 group">
      <span className="text-[10px] text-gray-500 font-mono tracking-widest uppercase group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors">{type}</span>
      <span className="text-lg font-mono font-bold uppercase tracking-tight text-black dark:text-white">{name}</span>
    </div>
  );
}
